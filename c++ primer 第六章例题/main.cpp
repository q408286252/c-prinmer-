...
int main()
{
    manip('a','z'); //实参都是char进行算术转换为int,int 优先级很差
    manip(55.4, dobj);  //实参都是浮点数也是要进行算术转换变成默认int
}
...
int main()
{
    ...
    //6.50
    f(2.56, 42) //可行函数为 void f(int,int); void f(double,double =3.15); 最终会拒绝,二义性
    f(42)   //可行函数为 coid f(int); void f(double,double =3.15) 会选f(int)
    f(42,0) //可行函数为 void f(int,int); void f(double,double =3.15); 会选f(int,int)
    f(2.56,3.14)    //可行函数为void f(int,int); void f(double,double =3.15); 会选f(double,double =3.15)


    //6.49
    //所有复合函数名的函数都属于候选函数,
    //可行函数就是函数所需的形参类型复合实参类型(可转换)
